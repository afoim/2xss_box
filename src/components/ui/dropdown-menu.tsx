...
/**
 * 无第三方依赖的下拉菜单：原生按钮 + createPortal 到 body + 手写定位。
 *
 * 纯 CSR 版本相对原仓库的唯一简化：那边为了 SSR 用了 useIsomorphicLayoutEffect
 * （服务端没有布局，React 会对 useLayoutEffect 告警）。这里没有服务端，
 * 直接用 useLayoutEffect —— 首帧就把位置量准，不会先画在 (0,0) 再跳。
 */
...
function DropdownMenuTrigger({ onClick, onKeyDown, ...props }: React.ComponentProps<'button'>) {
...
function DropdownMenuContent({
  ...
  const updatePosition = React.useCallback(() => {
    ...
    if (side === 'top') {
      ...
      if (top < 8 && rect.bottom + sideOffset + h <= vh - 8) top = rect.bottom + sideOffset;
    } else {
      top = rect.bottom + sideOffset;
      if (top + h > vh - 8 && rect.top - sideOffset - h >= 8) top = rect.top - sideOffset - h;
    }
