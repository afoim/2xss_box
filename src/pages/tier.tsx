  ...
  const exportImage = useCallback(async () => {
    ...
    try {
      ...
      track('使用工具', {
        工具: '梯度图',
        动作: '导出',
        张数: Object.values(board).reduce((a2, arr) => a2 + arr.length, 0),
      });
    } finally {
      setExporting(false);
    }
  ...
  return (
    <main className="mx-auto w-full max-w-4xl space-y-5 px-4 py-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="outline" onClick={clearAll} disabled={totalCount === 0} className="gap-1.5">
